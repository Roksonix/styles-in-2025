...
import {
  ...
  createPrimaryTextContainerStyles,
  createSecondaryTextContainerStyles,
  textContainerStyles,
} from "./QueryBarItemStyles";
 ...
 * (for secondary text)
 */
export const renderDefaultTextComponent = (
  isActive: boolean,
  text: string,
  secondaryText?: string,
  reverseEllipsis = false,
): ReactNode => {
  const textColor = isActive ? colorsPalettes.blue[400] : colorsPalettes.carbon[400];
  const hasSecondaryText = Boolean(secondaryText);

  return (
    <div className={textContainerStyles}>
      <div className={createPrimaryTextContainerStyles({ textColor, reverseEllipsis })}>
        <span>{text}</span>
      </div>
      {hasSecondaryText ? <div className={createSecondaryTextContainerStyles({ textColor })}>{secondaryText}</div> : null}
    </div>
  );
};
 ...
 */
export const renderDefaultIconComponent = (
  isActive: boolean,
  iconName: IconName,
  hideRemoveButton?: boolean,
): ReactNode => {
  const iconColor = isActive ? colorsPalettes.blue[400] : colorsPalettes.carbon[400];

  return (
    <div className={createIconContainerStyles({ iconColor, hideRemoveButton })}>
